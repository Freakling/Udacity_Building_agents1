...
class VectorStoreManager:
    ...
    def __init__(self, openai_api_key: Optional[str] = None, api_base: Optional[str] = None, use_local_embeddings: bool = False):
        self.chroma_client = chromadb.Client()
        if use_local_embeddings or not openai_api_key:
            self.embedding_function = embedding_functions.DefaultEmbeddingFunction()
        else:
            self.embedding_function = self._create_embedding_function(openai_api_key, api_base)

    def _create_embedding_function(self, api_key: str, api_base: Optional[str] = None) -> EmbeddingFunction:
...
class CorpusLoaderService:
    ...
    def load_json(self, store_name: str, json_path: str) -> VectorStore:
        """
        Load a JSON file containing game records into a vector store.

        Each game object is converted to a natural-language Document and
        embedded for semantic search. Compatible with the schema used by
        ``JSONGameLoader``.

        Args:
            store_name (str): Name of the vector store to create or reuse
            json_path (str): Path to the JSON file to load

        Returns:
            VectorStore: The populated vector store ready for querying

        Example:
            >>> loader = CorpusLoaderService(manager)
            >>> store = loader.load_json("games", "games.json")
            >>> results = store.query(["Who developed FIFA 21?"])
        """
        store = self.manager.get_or_create_store(store_name)
        print(f"VectorStore `{store_name}` ready!")

        loader = JSONGameLoader(json_path)
        corpus = loader.load()
        store.add(corpus)
        print(f"{len(corpus)} games from `{json_path}` added!")
